# sh-sparc.tcl - sparc-specific constants for host shell Tcl implementation
#
# Copyright 1994-1996 Wind River Systems, Inc. 
#
# modification history
# -----------------------
# 01s,01jul98,c_s  WindView 2 -> Tornado 2 Merge
# 01r,06may98,nps  added support for rBuff and triggering.
# 01t,05jun98,f_l  added offsets for taskWait routines
# 01s,17apr98,rlp  removed numMaxMsgsSent for backward compatibility.
# 01r,05nov97,rlp  added numMaxMsgsSent for tracking messages sent.
# 01q,29oct96,pad  Added routine underscoreStrip (SPR #7159).
# 01p,30sep96,pad  Added mangledSymListGet procedure
# 01o,29apr96,p_m  added pcGetGopher and spGetGopher to fix SPR#6409.
# 01n,06mar96,elp  changed trcSparcLib.tcl to dbgSparcLib.tcl.
# 01m,27feb96,elp  came back to version 01k, added availRegPuts{},
#		   regOffsetFromIndexGet{}, and source trcSparcLib.tcl.
# 01l,17nov95,elp  changed regSetGopher for mRegs{} changed regSetPrint{} and
#		   taskRegSetVec{} to take care of new regSetGopher.
# 01k,15nov95,c_s  added regSetOfContext.
# 01j,09nov95,s_w  added offset(WIND_TCB,safeCnt) to fix SPR 5090
# 01i,08nov95,elp  added regNList (register Name List) for mRegs
# 01h,10oct95,c_s  added exception printing capability.
# 01g,19sep95,pad  added offset information for Shared Memory Objects.
# 01f,12sep95,pad  added offset information for POSIX semaphores and msg queues.
# 01e,31may95,c_s  removed stackCheck gopher.
# 01d,14mar95,c_s  added pcOfRegSet.
# 01c,18jan95,c_s  fixed some offsets (old ones were for simsparc).
# 01b,15nov94,c_s  added WDOG offsets and a new SEMAPHORE offset.
...
#
# RESOURCE FILES
# wpwr/host/resource/tcl/dbgSparcLib.tcl
#

source "[wtxPath host resource tcl]dbgSparcLib.tcl"
...
set offset(WIND_TCB,pc)			0x130
set offset(WIND_TCB,sp)			0x188
...
proc excInfoShow {vec esf} {
    ...
    # A gopher for unpacking a SPARC fault record (see excSparcLib.h,
    # structure ESF
    #
    # Format: pc npc psr
    ...
    set excName [lindex $sparcExcName $vec]

    if {$vec == 0x83} {
	set excName "unrecoverable breakpoint hit at interrupt or kernel time"
    }
    if {$vec == 0x82} {
	set excName "Zero Divide"
    } 

    if {$excName == "NULL" || $excName == ""} {
	set excName "Trap to uninitialized vector number $vec"
    }

    set excInfo [wtxGopherEval "$esf $excGopher"]
    bindNamesToList {pc npc psr} $excInfo

    puts stdout "\n$excName"
    puts stdout [format "pc        : %#x" $pc]
    puts stdout [format "npc       : %#x" $npc]
    puts stdout [format "pcw       : %#x" $psr]
}
