...
PUBLIC void jWrapLog (int level, char*format, ...) {
  ...
  if (jWrapDbgValue < level) return;

#ifdef PTHREAD
  fprintf (jWrapDbgFile,"%6ld", pthread_self ());
#endif
  {
    if (!jWrapDbgFile) jWrapDbgFile=stdout;
 ...
 **-----------------------------------------------------------------------*/
LOCAL int jWrapCmd (ClientData cld, Tcl_Interp *interp
...

{
   ...
   static char   *arrayOptions[] 
     = {"help","info","struct","cmd","enum"
       ,"handle","debug","type","sizeof"
       ,"new","cast","cget","equal"
       ,"version","copyright","perror","convert","forget"
       ,(char *) NULL};
   ...
   if (moduleHead == NULL) goto errNoModule;

   // In all other case we check input parameters
   if (objc < 2) goto errSyntax;
   ...
   switch (ind) {
     ...
     case 1: // info
      ...
      if ((objc >2) && (module == NULL)) goto errModuleName;
      ...
      if  (module != NULL) {
        ...
        for (module = moduleHead; module != NULL; module = module->next) nbModule ++;
        { 
          ...
          if (listAsked) {
            ...
            for (ind= 0; ind < nbModule; ind ++) Tcl_Free (line [ind]);
          }
        } // end block
   ...
   case 3: // cmd
     ...
     case 5: // handle
        entryPtr = Tcl_FindHashEntry (&jWrapHashTable,TCL_STRING (objv[2]));
        if (entryPtr == NULL) goto errUnknowHandle;
	tclObj = Tcl_NewLongObj ((long)Tcl_GetHashValue (entryPtr));
     ...
     case 6: // debug
        ...
        if (Tcl_GetIntFromObj(interp, objv[objc-1],&jWrapDbgValue) != TCL_OK) goto errDbgLvl;
	if (objc == 4) {
     ...
     case 8: // sizeof
       ...
       if (entryPtr == NULL) goto errSizeof;
     ...
     case 9:  // new
       ...
       switch  (hashObj->magic) {
         ...
         case JWRAP_STRUCT:
           ...
           if (callBack == NULL) goto errCallBack;
     ...
     case 16: // convert from the external rep
       ...
       if (status != TCL_OK) goto errConvertType;
...
errModuleName:
   Tcl_AppendResult (interp,"\nERROR: Unknow jWrap Module [",TCL_STRING(objv[objc-1])
     ,"]",NULL);
   return (TCL_ERROR);
...
errUnknowHandle:
  Tcl_AppendResult (interp,"\nUnknown C++ Object in HashTable for jTcl handle="
                   ,TCL_STRING (objv[2]),NULL);
  return (TCL_ERROR);

errDbgLvl:
  Tcl_AppendResult (interp,"\nsyntaxe:= jWrap debug [file] level should be 0-9 not:"
                   ,TCL_STRING (objv[2]),NULL);
  return (TCL_ERROR);

errName:
  Tcl_AppendResult (interp,"\nERROR: jWrap new unknow structure:"
                   ,TCL_STRING (objv[2]),NULL);
  return (TCL_ERROR);

errDbgFile:
  Tcl_AppendResult (interp,"\njWrap Debug could not open debug log file:"
                   ,TCL_STRING (objv[2]),NULL);
  return (TCL_ERROR);
 ...
 errUnknowType:
   Tcl_AppendResult (interp,"ERROR: jWrap cast [", TCL_STRING(objv[2])
                    ,"] unknow type\n",NULL);
   return (TCL_ERROR);
 ...
 unknowObject:
   Tcl_AppendResult (interp,"ERROR: Invalid magic for hashtable object ["
                    ,hashObj->type->obj->name,"]\n",NULL);
   return (TCL_ERROR);
 ...
 errConvertType:
   Tcl_AppendResult (interp,"Failled to convert [",objv[3]->bytes
                    ,"] into [",objv[2]->bytes,"]\n",NULL); 
   return (TCL_ERROR);
...
PUBLIC void jWrapModuleRegister (Tcl_Interp *interp,JWRAP_modules *module) 
{
  ...
  if (module->magic != JWRAP_MODULE) goto errMagic; 
...
PUBLIC void jWrapExtendClass (Tcl_Interp *interp, char *extendClass, char *superClass) {
 ...
 if (superPtr == NULL) return;

 // get original class array
 extendPtr = TclLookupVar(interp, extendClass, (char *) NULL, TCL_GLOBAL_ONLY,
            /*msg*/ 0, /*createPart1*/ 0, /*createPart2*/ 0, &arrayPtr);
 if (extendPtr == NULL) goto invalidClass;
 
 // loop on super class hash and transfer to target class
 for ( superEntry  = Tcl_FirstHashEntry(superPtr->value.tablePtr, &search)
     ; superEntry != NULL
     ; superEntry  = Tcl_NextHashEntry(&search)) {
     ...
     if (!strcmp (TCL_STRING(jWrapMyString),slotName)) continue;
     if (!strcmp (TCL_STRING(jWrapExtendsString),slotName)) continue;
...
PUBLIC void jWrapMiscInit (Tcl_Interp *interp)
{
 ...
 int  status;
  ...
  Tcl_CreateObjCommand (interp,"jWrap", jWrapCmd
  ...
  if (loginfo != NULL) {
   ...
   if (jWrapDbgFile != NULL) goto errFile;
  }
